...
def get_neighbour_edge_index(data, self_loops=False):
    ...
    if len(new_to_nodes) > 0:
        ...
        return torch.tensor([])
def prepare_features(data,matches,n_f):
    homeFeatures=['FTHG','HS','HST','HF','HC','HY','HR']
    awayFeatures=['FTAG','AS','AST','AF','AC','AY','AR']
    homeFeatures=homeFeatures[:n_f]
    awayFeatures=homeFeatures[:n_f]
    n_features=len(homeFeatures)
    featuresHome=['HomeTeam']
    featuresAway=['AwayTeam']
    featuresHome.extend(homeFeatures)
    featuresAway.extend(awayFeatures)
    matches[homeFeatures]=matches[homeFeatures].astype(float)
    matches[awayFeatures]=matches[awayFeatures].astype(float)
    home_features=matches.loc[:,featuresHome].groupby(['HomeTeam']).sum()
    away_features=matches.loc[:,featuresAway].groupby(['AwayTeam']).sum()
    homeTeamFeatures=np.zeros((data.n_teams,n_features))
    awayTeamFeatures=np.zeros((data.n_teams,n_features))
    homeTeamFeatures[home_features.rename_axis('index').reset_index()['index'].to_list()]=home_features.to_numpy()
    awayTeamFeatures[away_features.rename_axis('index').reset_index()['index'].to_list()]=away_features.to_numpy()
    teams_features=homeTeamFeatures+awayTeamFeatures
    data.team_features= torch.from_numpy(teams_features).float()
